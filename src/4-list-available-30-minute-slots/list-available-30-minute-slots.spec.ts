...
export function listAvailable30MinuteSlots(
  ...
  for (
    ...
    dayAvailability.range.forEach(range => {
      ...
      // Check the end time of the range
      ...
      while (currentSlotStart < rangeEndTime) {
        const currentSlotEnd = new Date(currentSlotStart.getTime() + 30 * 60 * 1000); // 30 minutes slot

        // Ensure the slot is within the date range provided
        if (currentSlotStart < startRange || currentSlotEnd > endRange) {
          currentSlotStart = new Date(currentSlotEnd); // Move to the next slot
          continue; // Skip this iteration if the slot is out of range
        }
        ...
        const hasConflict = events.some(event => {
          ...
          return (
            currentSlotStart < eventEnd && currentSlotEnd > eventStart // Slot overlaps with event
          );
        });
